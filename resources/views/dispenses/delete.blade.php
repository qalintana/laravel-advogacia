    ...
    <div class="card card-blue">
      <div class="row">
        <div class="card-body">
          ...
          <form dispense="{{route('dispense.destroy', $dispense->id)}}" method="POST">
            <h2 class=" text-center">Tem certeza que deseja eliminar a despesa {{ $dispense->ato }}
            </h2>
            <hr>
            @method('DELETE')
            @csrf
            <div class="row justify-content-center flex-row">
              ...
              &nbsp;
              <a href="{{ route('dispense.index')}}" class="btn btn-primary btn-sm"> <i class="fas fa-not-equal"></i>
                Não </a>
              &nbsp;
              <a href="{{ route('dispense.show', $dispense->id)}}" class="btn btn-info btn-sm"> <i
                  class="fas details"></i>
                Ver detalhes </a>
            </div>
          </form>
